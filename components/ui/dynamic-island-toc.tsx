...
/**
 * Dynamic Island table of contents (21st.dev), adapted to the portfolio:
 * rebuilt on the theme tokens (--bg-elevated / --text-primary / --border-default
 * / --pill-bg-active) so it flips light/dark, `@/lib/cn`, and a default
 * `[data-toc]` selector so it only picks up explicitly-marked section anchors.
 * Fixed at the bottom centre, so it sits over content without affecting layout.
 */
...
type DynamicIslandTOCProps = {
  children?: ReactNode;
  /** CSS selector to find headings. Defaults to explicit [data-toc] anchors. */
  selector?: string;
};
...
export function DynamicIslandTOC({
  ...
  // No anchors found (e.g. wrong page) - render nothing.
    ...
    <>
      ...
      >
        <motion.div
          ...
          className="relative overflow-hidden border border-[var(--border-default)] bg-[var(--bg-elevated)] text-fg shadow-2xl"
        >
          {/* Closed pill */}
          <motion.div
            initial={false}
            ...
            data-cursor-text="Contents"
          >
            <div className="h-2 w-2 shrink-0 rounded-full bg-[var(--text-primary)]" />
            ...
            <div className="relative flex h-full flex-1 items-center overflow-hidden text-left">
              <AnimatePresence mode="popLayout" initial={false}>
                <motion.span
                  ...
                  className="block w-full overflow-hidden text-ellipsis whitespace-nowrap text-sm font-medium text-fg"
                >
          ...
          >
            <div className="flex shrink-0 items-center justify-between px-6 pb-3 pt-5">
              <span className="font-[family-name:var(--font-inter)] text-[11px] font-semibold uppercase tracking-[0.08em] text-fg-subtle">
                Table of contents
              </span>
              <button
                onClick={(e) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        isActive && "bg-[var(--pill-bg-active)] font-medium text-fg",
                        !isActive && isHovered && "bg-[var(--bg-muted)] text-fg",
                        !isActive && !isHovered && "bg-transparent text-fg-subtle",
                      )}
                    >
                      ...
                      <motion.div
                        ...
                        className="ml-3 h-1.5 w-1.5 shrink-0 rounded-full bg-[var(--text-primary)]"
                      />
                    </button>
                  );
                })}
